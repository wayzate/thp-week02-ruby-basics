...
puts handles.min_by{ |handle| handle.length }
puts

puts "=== Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"
puts handles.select{ |handle| handle.length == 6}.length
puts
...
puts handles.sort_by{ |handle| handle.length}
puts
...
for i in 1..(handles.max_by{ |handle| handle.length }.length) do
  puts "===--- Handles à #{i-1} caractères"
  puts handlesOfLengthI = handles.select{ |handle| handle.length == i }.length
end
